...
const ViewBid = () => {
    ...
    const fetchData = () => {
        ...
        const url = process.env.REACT_APP_MAIN_URL + "/dashboard/bids";
        axios
            .get(url, config)
            .then((response) => {
    ...
    const renderbids = bids && bids.map((bid, index) => {
        return (
            <tr key={index}>
                <th>{index + 1}</th>
                <th>
                    <div className="dropdown">
                        <a id="navbarDropdown" className="dropdown-toggle" role="button"
                        ...
                        <div className="dropdown-menu dropdown-menu-start"
                            ...
                            <Link className='dropdown-item text-end' to={`/ShowBid/${bid.id}`}>
                                <i className="fa fa-eye text-black-50 mx-2"></i>عرض</Link>
                            <Link className='dropdown-item text-end' to={`/ViewProposals/${bid.id}`}>
                                <i className="fa fa-pen text-black-50 mx-2"></i>العروض</Link>
                            <Link className='dropdown-item text-end' to={`/EditBid/${bid.id}`}>
                                <i className="fa fa-pen text-black-50 mx-2"></i>تعديل</Link>
                            <DeleteButton fetchData={fetchData} id={bid.id} name={bid.name} url='bids' key={index} />
                        </div>
                    </div>
                </th>
                <th scope="row" >{bid.title}</th>
                <th scope="row" >{bid.cityname}</th>
