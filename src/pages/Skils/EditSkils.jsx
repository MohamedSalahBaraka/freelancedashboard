...
const CreateCity = () => {
    ...
    const fetchData = () => {
        ...
        const url = process.env.REACT_APP_MAIN_URL + "/dashboard/skils/" + id;
        axios
            .get(url, config)
            .then((response) => {
                reset(response.data.skil)
            })
            .catch((error) => {
                console.log(error);
    ...
    const onSubmit = (data) => {
        ...
        let url = process.env.REACT_APP_MAIN_URL + "/dashboard/skils";
        let token = localStorage.getItem('token');
        const config = {
            headers: {
                "Authorization": "Bearer " + token,
        ...
        axios
            ...
            .then((response) => {
                setseccuss("City Updated successfully!");
                seterror(false);
            })
            .catch((error) => {
                setseccuss(false);
    ...
    return (
        <div className="d-flex w-100 flex-column align-items-center justify-content-center mt-4">
            ...
            <div className="h1">عدل مهارة</div>
