...
const ViewCity = () => {
    ...
    const fetchData = () => {
        ...
        const url = process.env.REACT_APP_MAIN_URL + "/dashboard/city";
        axios
            .get(url, config)
            .then((response) => {
    ...
    const renderCities = cities && cities.map((city, index) => {
        return (
            <tr key={index}>
                <th>{index + 1}</th>
                <th>
                    <div className="dropdown">
                        ...
                        <div className="dropdown-menu dropdown-menu-start"
                            ...
                            <Link className='dropdown-item text-end' to={`/ShowCity/${city.id}`}>
                                <i className="fa fa-eye text-black-50 mx-2"></i>عرض</Link>
                            <Link className='dropdown-item text-end' to={`/EditCity/${city.id}`}>
                                <i className="fa fa-pen text-black-50 mx-2"></i>تعديل</Link>
                            <DeleteButton fetchData={fetchData} id={city.id} name={city.name} url='city' key={index} />
                        </div>
                    </div>
                </th>
        ...
        <div className="container d-flex flex-column flex-grow-1 flex-wrap justify-content-center align-items-center">
            <div className="container pt-3">
                <div className="row justify-content-center">
                    <label htmlFor="floatingInput " className="h1">جميع المدن</label>
