...
const ViewPhones = () => {
    ...
    const fetchData = () => {
        ...
        const url = process.env.REACT_APP_MAIN_URL + "/dashboard/phone";
        axios
            .get(url, config)
            .then((response) => {
    ...
    const renderphones = phones && phones.map((phone, index) => {
        return (
            <tr key={index}>
                <th>{index + 1}</th>
                <th>
                    <div className="dropdown">
                        ...
                        <div className="dropdown-menu dropdown-menu-start"
                            ...
                            <Link className='dropdown-item text-end' to={`/ShowPhones/${phone.id}`}>
                                <i className="fa fa-eye text-black-50 mx-2"></i>عرض</Link>
                            <Link className='dropdown-item text-end' to={`/EditPhones/${phone.id}`}>
                                <i className="fa fa-pen text-black-50 mx-2"></i>تعديل</Link>
                            <DeleteButton fetchData={fetchData} id={phone.id} name={phone.number} url='phone' key={index} />
                        </div>
                    </div>
                </th>
        ...
        <div className="container d-flex flex-column flex-grow-1 flex-wrap justify-content-center align-items-center">
            <div className="container pt-3">
                <div className="row justify-content-center">
                    <label htmlFor="floatingInput " className="h1">جميع ارقام الهواتف</label>
